...
/* TO MODIFY */
        bool getObjective(eurobot2014_petit_ai::GetObjective::Request  &req, eurobot2014_petit_ai::GetObjective::Response &res );
        bool updateObjective(eurobot2014_petit_ai::UpdatePriority::Request  &req, eurobot2014_petit_ai::UpdatePriority::Response &res );
/*****END*****/
...
/* TO MODIFY */
    goal_debug = nh.advertise < geometry_msgs::PoseStamped > ("/PETIT/best_objective", 10);
    path_debug = nh.advertise < nav_msgs::Path > ("/PETIT/debug_path", 10);

    get_pose = nh.serviceClient<common_smart_nav::GetRobotPose>("/PETIT/get_robot_pose");
    get_path = nh.serviceClient<nav_msgs::GetPlan>("/move_base_PETIT/NavfnROS/make_plan"); // ???????
    get_distance = nh.serviceClient<common_smart_nav::GetDistance>("/PETIT/get_distance");

    goal_service = nh.advertiseService("/PETIT/get_objective", &ObjectiveManager::getObjective, this);

    delet_sub = nh.subscribe < geometry_msgs::PoseStamped > ("/PETIT/delet_objective", 10, &ObjectiveManager::deletCallback, this);

    update_prio_service = nh.advertiseService("/PETIT/update_priority", &ObjectiveManager::updateObjective, this);
...
{   
    ...
    tmp_obj.pose.position.y = 1.55; 
    // Priority : 60
    objectives.push_back( pair<geometry_msgs::PoseStamped, uint32_t>(tmp_obj, 60) );

    tmp_obj.pose.position.x = color*(0.150); // frescoes
    tmp_obj.pose.position.y = 1.800;
    // Priority : 30
    objectives.push_back( pair<geometry_msgs::PoseStamped, uint32_t>(tmp_obj, 30) ); 

    tmp_obj.pose.position.x = -color*(0.750); // opponent mammouth
    tmp_obj.pose.position.y = 1.55;
    // Priority : 20
...
/* TO MODIFY */
bool ObjectiveManager::getObjective(eurobot2014_petit_ai::GetObjective::Request  &req,
        eurobot2014_petit_ai::GetObjective::Response &res )
{
/*****END*****/
...
/* TO MODIFY */
bool ObjectiveManager::updateObjective(eurobot2014_petit_ai::UpdatePriority::Request  &req, eurobot2014_petit_ai::UpdatePriority::Response &res )
{
/*****END*****/
...
int main(int argc, char **argv)
{
     ...
     */
/* TO MODIFY */
    ros::init(argc, argv, "PETIT_objective_manager");
/* TO MODIFY */
    ObjectiveManager objectivemanager;
