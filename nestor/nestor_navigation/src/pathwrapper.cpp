...
Pathwrapper::Pathwrapper()
{
    ...
    pose2D_pub = nh.advertise < geometry_msgs::Pose2D > ("/nestor_goal", 1);
    poseArray_pub = nh.advertise < geometry_msgs::PoseArray > ("/poses", 5);
			...
			if( !(my_path.poses.std::vector<geometry_msgs::PoseStamped >::empty()) ){
				if(my_path.poses.std::vector<geometry_msgs::PoseStamped >::size() > NB_STEP_SKIP) {
					...
					if( my_pose_stamped.pose.position.x > my_pose_stamped.pose.position.y)
					{
						...
						double max_speed_lin = my_pose_stamped.pose.position.x;
						if(max_speed_lin > 0.1)
							max_speed_lin = 0.1;
						if(max_speed_lin < -0.1)
                                                        max_speed_lin = -0.1;
					...
					{       
						...
						double max_speed_lin = my_pose_stamped.pose.position.y;
                                                if(max_speed_lin > 0.1)
                                                        max_speed_lin = 0.1;
                                                if(max_speed_lin < -0.1)
                                                        max_speed_lin = -0.1;
					...
					// find Vtheta
    					final_cmd_vel.angular.z = getHeadingFromQuat(my_pose_stamped.pose.orientation); // RAD ?
					if(final_cmd_vel.angular.z > 0.15)
