...
void nestorWeather::weatherCallback(const std_msgs::Empty::ConstPtr & feedback)
{
	...
	/* Open the command for reading. */
  	fp = popen("~/ROS/smart-robotics-ros-pkg/nestor/nestor_command/current.sh \"EUR|FR|FR|Brunoy\"", "r");
  	if (fp == NULL) {
...
*/
  	...
  	pclose(fp);
        ...
        /* Forecast weather */   
        /* Open the command for reading. */
        fp = popen("~/ROS/smart-robotics-ros-pkg/nestor/nestor_command/forecast.sh \"EUR|FR|FR|Brunoy\"", "r");
        if (fp == NULL) {
                printf("Failed to run command\n" );
                exit;
        }
